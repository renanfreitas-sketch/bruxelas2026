...
Busca dados do board monday.com 18418387928 e gera index.html atualizado.
...
def build_html(items_raw):
    ...
    items = []
    for it in items_raw:
        # only items in group "topics" (Respostas Recebidas) – filter by name not being blank
...
<section class="hero">
  <div class="hero-inner">
    ...
    <h1>Relatório de Avaliação<br><span>World of Coffee Bruxelas</span></h1>
    <p>Resultados consolidados das avaliações recebidas das empresas participantes da missão comercial à World of Coffee Brussels 2026.</p>
  </div>
</section>
...
<footer class="site-footer">
  ...
  <p>BSCA – Associação Brasileira de Cafés Especiais &nbsp;·&nbsp; Projeto Brazil. The Coffee Nation &nbsp;·&nbsp; World of Coffee Brussels 2026</p>
</footer>
...
def push_to_github(html_content):
    ...
    GH_REPO  = os.environ.get("GITHUB_REPOSITORY", "renanfreitas-sketch/bruxelas2026")
    BRANCH   = os.environ.get("GITHUB_REF_NAME", "main")

    # Get current SHA
    req_get = urllib.request.Request(
        f"https://api.github.com/repos/{GH_REPO}/contents/index.html",
        headers={
    ...
    payload = {
        "message": f"dashboard: auto-update {now}",
    ...
    with urllib.request.urlopen(req_put) as resp:
        ...
        print(f"Pushed: {result.get('content', {}).get('html_url', 'OK')}")
...
if __name__ == "__main__":
    ...
    if "--dry-run" in sys.argv:
        with open("index.html", "w", encoding="utf-8") as f:
            f.write(html)
        print("Saved to index.html (dry run)")
    else:
        push_to_github(html)
        print("Done.")
